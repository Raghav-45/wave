...
export default function Chats() {
  ...
  return (
    <Box height={'100vh'} width={'100%'} px={'20px'} pt={'24px'} bgGradient='linear(to-tr, rgb(38 41 47 / 03%), rgb(53 61 76 / 06%))'>
      ...
      {isUserListLoading ? <Text textAlign={'center'} width={'100%'}>Loading chats... <Spinner height={'3'} width={'3'} color='#0071FF' /></Text> : userList.map((elem) => elem.id != currentUser.id && <Chat Username={elem?.username} />)}
    </Box>
  )
}
